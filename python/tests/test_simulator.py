...
class SyntheticReplayTests(unittest.TestCase):
    ...
    def test_end_to_end_backtest_is_deterministic(self) -> None:
        ...
        self.assertGreaterEqual(first.gross_spread_capture_pnl, 0)
        self.assertGreaterEqual(first.negative_markout_loss, 0)
    ...
    def test_queue_proxy_increases_fill_chance_for_deeper_crossing(self) -> None:
        ...
        self.assertLess(
            model.fill_probability(at_touch, quote),
            model.fill_probability(deep_cross, quote),
        )
